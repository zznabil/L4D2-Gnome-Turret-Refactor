...
{
	if (!hEntity || !hTarget) return printl("[GetAngleBetweenEntities] Entity doesn't exist");
...
function OnTickCall()
{
	...
	// Throttle convar change detection to every 0.1s (was running every tick)
	if (g_flLastConVarCheckTime < Time())
	{
		g_flLastConVarCheckTime = Time() + 0.1;
	foreach (cvar in g_aConVars)
	{
...
function ButtonsListener_Think()
{
	...
	{
		local hPlayer, team, buttonsPressed, buttonsReleased, buttonsHeld;
		while (hPlayer = Entities.FindByClassname(hPlayer, "player"))
		{
			// Cache all button states once per player per tick (3 NetProps calls vs N*3)
			buttonsPressed = NetProps.GetPropInt(hPlayer, "m_afButtonPressed");
			buttonsReleased = NetProps.GetPropInt(hPlayer, "m_afButtonReleased");
			buttonsHeld = NetProps.GetPropInt(hPlayer, "m_nButtons");
			foreach (button in g_aButtonsListener)
			{
				team = button.GetTeam();
				if (team == eTeam.Everyone) CheckButtonsCached(hPlayer, button, buttonsPressed, buttonsReleased, buttonsHeld);
				else if (team == eTeam.Survivor && hPlayer.IsSurvivor()) CheckButtonsCached(hPlayer, button, buttonsPressed, buttonsReleased, buttonsHeld);
				else CheckButtonsCached(hPlayer, button, buttonsPressed, buttonsReleased, buttonsHeld);
			}
		}
	}
}

function CheckButtonsCached(hPlayer, button, buttonsPressed, buttonsReleased, buttonsHeld)
{
	local button_type = button.GetType();
	local buttons;
	if (button_type == eButtonType.Pressed) buttons = buttonsPressed;
	else if (button_type == eButtonType.Released) buttons = buttonsReleased;
	else buttons = buttonsHeld;
