...
if (!("g_RoundState" in getroottable()))
	::g_RoundState <- {};
g_RoundState.smStartboxList <- []
g_RoundState.smStartboxCenter <- null
g_RoundState.smStartboxRemoveAll <- true
...
function CreateSingleSimpleEntityFromTable( spawnTable, entityAt = null )
{
	...
	if (entityAt != null)
	{
		...
		singleSimpleShell.EntityGroup.SpawnTables.singlesimple.SpawnInfo.angles = Vector(entityAt.GetAngles().x, entityAt.GetAngles().y, entityAt.GetAngles().z)
	}
...
IncludeScript("turret", getroottable());
IncludeScript("entity_pool", getroottable());
InitParticlePools();
