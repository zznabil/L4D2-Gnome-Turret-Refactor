...
g_flGnomeTurretSweepSpeed <- 20.0;
g_flGnomeTurretSweepArc <- 90.0;
g_bTurretParticlesEnabled <- 1;

GnomeLimit <- 2
DemolitionShot <- 1
ExplosionAmmoToggle <- 0
g_bDemolitionMode <- false
...
"TurretParticlesEnabled 1",
		...
		".",
		"//GnomeTurretAmmoBase= This controls gnome turret ammo. Default value is 300. Minimum value is 5, so setting the value to lower than 5 will make gnome turret have 5 ammo."
		"Below 5= Gnome turret is 5."
		"higher than 5= Gnome turret ammo (1 * value)."
		"//GnomeTurretSweepSpeed= Controls how fast the turret sweeps when idle. Measured in degrees per second. Default 20.",
		"//GnomeTurretSweepArc= Controls the total sweep arc width. Default 90 (sweeps +-45 degrees from center).",
		".",
		".",
		"// =================================",
	...
	foreach(toggle in toggles)
	{
		if(toggle && toggle != "")
		{
			...
			if (idx != null)
			{
				...
				{
					GnomeTurretAmmoBase = ClampAmmo(togglevalue.tointeger());
				}
				if(togglecommand == "GnomeTurretSweepSpeed")
				{
					local val = togglevalue.tofloat();
					if(val < 1.0) val = 20.0;
					if(val > 360.0) val = 20.0;
					g_flGnomeTurretSweepSpeed = val;
				}
				if(togglecommand == "GnomeTurretSweepArc")
				{
					local val = togglevalue.tofloat();
					if(val < 2.0) val = 90.0;
				...
				if(togglecommand == "TurretParticlesEnabled")
				{
...
// Export shared functions to root scope so turret.c/lib_utils.c can call them
::LoadSpecificConfigFile <- LoadSpecificConfigFile;
::CfgFileCheck <- CfgFileCheck;
::GenerateGnomeTurretCfgFile <- GenerateGnomeTurretCfgFile;
::GenerateGnomeVirtualInventory <- GenerateGnomeVirtualInventory;
::GenerateGnomeVirtualInventoryReset <- GenerateGnomeVirtualInventoryReset;
::IsCertainSurvivor <- IsCertainSurvivor;
::GetItemAmmo <- GetItemAmmo;
::SetItemAmmo <- SetItemAmmo;
::GetButtonPressed <- GetButtonPressed;
::ShowSpecialHint <- ShowSpecialHint;
::GetSecondarySlot <- GetSecondarySlot;
::ForcedToSwitchSecondary2 <- ForcedToSwitchSecondary2;

// Export shared globals
...
::g_bDemolitionMode <- g_bDemolitionMode
::g_iTurretCount <- g_iTurretCount
::g_iMaxTurrets <- g_iMaxTurrets
...
::EllisModel <- EllisModel;
::RochelleModel <- RochelleModel;
::BillModel <- BillModel;
::LouisModel <- LouisModel;
::FrancisModel <- FrancisModel;
::ZoeyModel <- ZoeyModel;

// Export icon constants
...
::g_flGnomeTurretSweepSpeed <- g_flGnomeTurretSweepSpeed;
::g_flGnomeTurretSweepArc <- g_flGnomeTurretSweepArc;
::g_bTurretParticlesEnabled <- g_bTurretParticlesEnabled;
